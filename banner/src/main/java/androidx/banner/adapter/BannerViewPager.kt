...
internal class BannerViewPager
@JvmOverloads constructor(
    context: Context,
    attrs: AttributeSet? = null
) : ViewPager(context, attrs) {

    var viewTouchMode = false
        set(value) {
            field = value
    ...
    override fun onInterceptTouchEvent(event: MotionEvent) = if (isVertical) {
    ...
    override fun onTouchEvent(ev: MotionEvent) = if (isVertical) {
        super.onTouchEvent(swapEvent(ev))
    } else {
        super.onTouchEvent(ev)
    }

    override fun arrowScroll(direction: Int) = !viewTouchMode
